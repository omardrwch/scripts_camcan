"""
Apply MF analysis to EOG channels of CamCAN dataset.
"""
import numpy as np
import camcan_utils
...
output_dir = op.join(mf_io_info['camcan_output_dir'], 'EOG')
...
# MF analysis
#-------------------------------------------------------------------------------
def single_eog_mf_analysis(args):
    """
    ...
    try:
        ...
        for params_index in params_index_list:
            ...
            output_filename = os.path.join(subject_output_dir, condition + "_channel_%s_params_%d"%('EOG', params_index) +'.h5')
...
if __name__ == '__main__':
    ...
    for args in arg_instances:
        subject, condition, params_index_list, max_j = args
        subject_output_dir = os.path.join(output_dir, subject)
        output_filename = os.path.join(subject_output_dir, condition + "_channel_%s_params_%d"%('EOG', params_index_list[-1]) +'.h5')
    ...
    Parallel(n_jobs=1, verbose=1, backend="threading")(map(delayed(single_eog_mf_analysis), arg_instances))
